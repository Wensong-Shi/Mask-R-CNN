...
class MaskRCNN(nn.Module):
    ...
    def forward(self, img, img_info, gt_labels, gt_bboxes, gt_masks):
        ...
        # forward(eval)
            ...
            assert batch_size == 1, 'Batch size should be 1 in eval mode!'
            # roi align(fc)
            features_pooled_fc = features[0].new_zeros([proposals.size()[0], 256, 7, 7])
            for i, stride in zip(range(4), cfg.FEATURE_STRIDES[:4]):
                index = level_proposals == i
            ...
            x_reg = self.fc_reg(features_pooled_fc)
            # get labels
            prob, labels_pred = torch.max(prob, dim=1)
            # format reg
            x_reg = x_reg.view(-1, cfg.NUM_CLASSES, 4)
            x_reg = torch.gather(x_reg, 1, labels_pred.view(-1, 1, 1).expand(-1, -1, 4).long())
            x_reg = x_reg.view(-1, 4)
            # get bboxes
            proposals = bbox.reg2bbox(proposals[:, 1:], x_reg)
            prob, proposals = bbox.clip_bbox(prob, proposals, img_info[0])
            # choose top 100 proposals
            _, index = torch.sort(prob, descending=True)
            index = index[:100]
            labels_pred_cal_masks = labels_pred[index]
            proposals_cal_masks = proposals[index]
            level_proposals = level_proposals[index]
            # roi align(conv)
            features_pooled_conv = features[0].new_zeros([proposals_cal_masks.size()[0], 256, 14, 14])
            for i, stride in zip(range(4), cfg.FEATURE_STRIDES[:4]):
                index = level_proposals == i
                if index.any():
                    features_pooled_conv[index] = roi_align(features[i], [proposals_cal_masks[index]], 14, 1 / stride)
            # head(conv)
            features_pooled_conv = self.head_mask_rcnn(features_pooled_conv)
            # output(conv)
            masks_pred = self.conv_mask(features_pooled_conv)
            # format mask
            masks_pred = masks_pred.view(-1, cfg.NUM_CLASSES, 28 * 28)
            masks_pred = torch.gather(masks_pred, 1,
                                      labels_pred_cal_masks.view(-1, 1, 1).expand(-1, -1, 28 * 28).long())
            masks_pred = masks_pred.view(-1, 28, 28).sigmoid()
            """
            prob: TOP_N_POST_NMS_TEST, [prob0, prob1, ...], torch.float32
            labels_pred: TOP_N_POST_NMS_TEST, [label1, label2, ...], torch.int32
            proposals: TOP_N_POST_NMS_TEST * 4, [[x1, y1, x2, y2], [x1, y1, x2, y2], ...], torch.float32
            masks_pred: 100 * 28 * 28, torch.float32
            """
            return prob, labels_pred, proposals, masks_pred
